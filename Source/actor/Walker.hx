...
class Walker implements Object_{
	...
	public var moveFrequency:Int = 2;

	private var drawArea:Rectangle;
	private var fileName:String = "c_slime.png";
	...
	public function new (grid:Point,ID:Int,d:String){
		this.ID = ID;
		logical.x = 32*grid.x;
		logical.y = 32*grid.y;
		dest.x = grid.x;
		...
		update();
		tips = new Window(container.x, container.y, 100, 36, true);
	...
	private function hideTips( e:MouseEvent ):Void{
		if(container.parent != null)container.parent.parent.removeChild(tips.container);
	}
